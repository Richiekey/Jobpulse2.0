...
export class ApplyToJobAdapter implements ATSAdapter {
  public readonly platformSlug = 'applytojob';
  public readonly parserVersion = 'applytojob_v1';

  public detect(url: string): ATSDetectionResult {
    const pattern = /([a-zA-Z0-9_-]+)\.applytojob\.com/i;
  ...
  public async validateSource(config: CompanySourceConfig): Promise<SourceValidationResult> {
    ...
    const url = `https://${slug}.applytojob.com/apply`;
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        ...
        const matches = Array.from(html.matchAll(/href="([^"']*\/apply\/([a-zA-Z0-9_-]+))"/gi));
        const count = matches.length;

        return {
          isValid: count > 0 || html.includes('applytojob'),
          atsType: 'applytojob',
          boardIdentifier: slug,
          jobsDiscoveredCount: count,
          sampleJobTitles: count > 0 ? ['Sample JazzHR Position'] : [],
          durationMs,
        };
      }
      ...
      return {
        ...
        sampleJobTitles: [],
        error: `ApplyToJob returned HTTP ${response.status}`,
        durationMs,
      };
    } catch (err: unknown) {
      return {
  ...
  public async discover(companySource: CompanySourceConfig): Promise<JobCandidate[]> {
    ...
    try {
      ...
      const regex = /href="([^"']*\/apply\/([a-zA-Z0-9_-]+))"[^>]*>([^<]+)<\/a>/gi;
      let match;
      while ((match = regex.exec(html)) !== null) {
        ...
        const fullUrl = link.startsWith('http')
          ? link
          : `https://${slug}.applytojob.com/apply/${jobId}`;
  ...
  public async parse(rawPayload: RawJobPayload): Promise<RawJob> {
    ...
    const applyUrl = data.url || `https://${rawPayload.sourceId}.applytojob.com/apply/${externalJobId}`;
    ...
    return {
      ...
      discoveryUrl: `https://${rawPayload.sourceId}.applytojob.com/apply`,
    };
  }
  ...
  public async resolveApplicationUrl(candidate: JobCandidate, raw: RawJob): Promise<string> {
    ...
    return `https://${candidate.companyIdentifier}.applytojob.com/apply/${candidate.externalJobId}`;
  }
}
