import type { JobCandidate, RawJobPayload } from '@jobpulse/domain';
...
function chooseDirectUrl(value: unknown): string | undefined {
  if (!isExternalHttpUrl(value)) return undefined;
  return value.trim();
}
...
function extractHelperPayload(html: string): Record<string, unknown> | null {
  ...
  try {
    ...
    return jobResult && typeof jobResult === 'object'
...
async function ensureJobrightSession(): Promise<string | null> {
  ...
  if (!email || !password) {
    logger.warn('jobright_direct_url_credentials_missing');
    return null;
  }
  ...
  try {
    ...
    logger.info('jobright_direct_url_session_authenticated');
...
async function resolveOriginalJobUrl(jobId: string): Promise<string | undefined> {
  ...
  const detailUrl = `${JOBRIGHT_DETAIL_PREFIX}${encodeURIComponent(jobId)}`;

  try {
    const response = await httpClient.get<string>(detailUrl, {
      timeoutMs: 15000,
    ...
    const helper = extractHelperPayload(response.data);
    ...
    return extractJsonLdUrl(response.data);
  } catch (error) {
    logger.warn('jobright_direct_url_detail_fetch_failed', {
      jobId,
      error: error instanceof Error ? error.message : String(error),
    });
    return undefined;
...
/**
 * Thin enrichment layer over the existing Jobright adapter.
 * It preserves the current README parser and adds the authenticated
 * Jobright detail-page lookup used by the legacy JobPulse implementation.
 */
export class JobrightDetailEnrichedAdapter extends JobrightAdapter {
  public readonly parserVersion = 'jobright_v3_direct_ats';
  ...
  public override async fetch(candidate: JobCandidate): Promise<RawJobPayload> {
    ...
    if (!existingDirectUrl && this.detailFetches < this.detailFetchCap) {
    ...
    return super.fetch({ ...candidate, payload });
  }
}
