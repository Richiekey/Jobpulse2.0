...
export class JobrightAdapter implements ATSAdapter {
  ...
  public detect(url: string): ATSDetectionResult {
    ...
    if (ghMatch && ghMatch[1]) {
      return { detected: true, atsType: 'jobright', boardIdentifier: ghMatch[1], confidence: 0.99, sourceUrl: url };
    }
    const jobPattern = /jobright\.ai\/jobs\/(?:info\/)?([a-zA-Z0-9_-]+)/i;
    const match = url.match(jobPattern);
    if (match && match[1]) {
      return { detected: true, atsType: 'jobright', boardIdentifier: match[1], confidence: 0.95, sourceUrl: url };
    }
  ...
  private async fetchReadmeContent(repoName: string): Promise<{ content: string; url: string; branch: string }> {
    ...
    try {
      const res = await httpClient.get<string>(masterUrl, { timeoutMs: 15000, maxSizeBytes: 20 * 1024 * 1024 });
      if (res.status === 200 && typeof res.data === 'string' && res.data.length > 0) return { content: res.data, url: masterUrl, branch: 'master' };
    } catch {
      // Fallback to main branch
    }
    const mainUrl = `https://raw.githubusercontent.com/jobright-ai/${cleanRepo}/main/README.md`;
    try {
      const mainRes = await httpClient.get<string>(mainUrl, { timeoutMs: 15000, maxSizeBytes: 20 * 1024 * 1024 });
      if (mainRes.status === 200 && typeof mainRes.data === 'string' && mainRes.data.length > 0) return { content: mainRes.data, url: mainUrl, branch: 'main' };
    } catch {
  ...
  public async validateSource(config: CompanySourceConfig): Promise<SourceValidationResult> {
    ...
    try {
      ...
      return { isValid: parseResult.candidates.length > 0, atsType: 'jobright', boardIdentifier: repo, jobsDiscoveredCount: parseResult.candidates.length, sampleJobTitles: parseResult.candidates.slice(0, 3).map((c) => c.title), durationMs: Date.now() - start };
    } catch (err: any) {
      return { isValid: false, atsType: 'jobright', boardIdentifier: repo, jobsDiscoveredCount: 0, sampleJobTitles: [], error: err.message || 'Validation request failed', durationMs: Date.now() - start };
    }
  }
  ...
  public async discover(companySource: CompanySourceConfig): Promise<JobCandidate[]> {
    ...
    const start = Date.now();
    logger.info('jobright_repository_fetch_started', { repository: repo, sourceId: companySource.sourceId });
    let readmeData: { content: string; url: string; branch: string };
    try {
      readmeData = await this.fetchReadmeContent(repo);
      logger.info('jobright_repository_fetch_succeeded', { repository: repo, branch: readmeData.branch, bytes: readmeData.content.length, durationMs: Date.now() - start });
    } catch (err: any) {
      logger.error('jobright_repository_fetch_failed', { repository: repo, error: err.message, durationMs: Date.now() - start });
      throw err;
    }
    const { candidates, rowsParsed, rowsRejected } = this.parseMarkdownTable(readmeData.content, repo, new Date(), readmeData.url);
    logger.info('jobright_candidates_created', { repository: repo, rowsParsed, rowsRejected, discoveredCount: candidates.length, sourceId: companySource.sourceId });
  ...
  public async fetch(candidate: JobCandidate): Promise<RawJobPayload> {
    if (!candidate.payload || typeof candidate.payload !== 'object' || Object.keys(candidate.payload).length === 0) {
      throw new Error(`Jobright fetch failed: Candidate ${candidate.externalJobId} is missing candidate.payload. Jobright adapter strictly requires candidate payload from GitHub discovery with 0 network requests.`);
    }
    ...
    return { sourceId: candidate.sourceId, externalId: candidate.externalJobId, payload, payloadHash, parserVersion: this.parserVersion, fetchedAt: new Date().toISOString() };
  }
  ...
  public async parse(rawPayload: RawJobPayload): Promise<RawJob> {
    ...
    const location = data.location ? [data.location as string] : Array.isArray(data.locations) ? (data.locations as string[]) : [];
    const description = (data.description as string) || `${title} at ${company}${location.length > 0 ? ` in ${location.join(', ')}` : ''}. Discovered via Jobright collection (${(data.repository as string) || 'Tech'}).`;
    ...
    return {
      sourceId: rawPayload.sourceId, externalJobId: externalId, rawTitle: title, rawCompany: company, rawDescription: description, rawDescriptionHtml: null,
      rawLocations: location, rawSalary: (data.salary as string) || undefined, rawPostedAt, rawEmploymentType, rawWorkplaceType, rawApplyUrl, sourceJobUrl, discoveryUrl,
      sourceMetadata: { companyName: company, companyWebsite: data.companyWebsite, repository: data.repository, originalSource: 'jobright_github_markdown', original_apply_url: data.original_apply_url, ats_url: data.ats_url, embedded_urls: data.embedded_urls },
    };
  }

  public async normalize(rawJob: RawJob, payloadHash: string): Promise<NormalizedJob> {
    const candidates: UrlCandidate[] = [];
    if (rawJob.sourceMetadata?.['original_apply_url']) candidates.push({ url: rawJob.sourceMetadata['original_apply_url'] as string, sourceType: 'explicit_employer_apply', suggestedConfidence: 0.98 });
    if (rawJob.sourceMetadata?.['ats_url']) candidates.push({ url: rawJob.sourceMetadata['ats_url'] as string, sourceType: 'explicit_ats_form', suggestedConfidence: 0.95 });
    if (Array.isArray(rawJob.sourceMetadata?.['embedded_urls'])) {
      for (const u of rawJob.sourceMetadata['embedded_urls']) if (typeof u === 'string' && u) candidates.push({ url: u, sourceType: URLResolver.isDirectAtsUrl(u) ? 'known_ats_url' : 'other_valid_url', suggestedConfidence: URLResolver.isDirectAtsUrl(u) ? 0.75 : 0.60 });
    }
    if (rawJob.rawApplyUrl && !rawJob.sourceMetadata?.['original_apply_url'] && !rawJob.sourceMetadata?.['ats_url']) candidates.push({ url: rawJob.rawApplyUrl, sourceType: URLResolver.isDirectAtsUrl(rawJob.rawApplyUrl) ? 'explicit_ats_form' : 'other_valid_url', suggestedConfidence: URLResolver.isDirectAtsUrl(rawJob.rawApplyUrl) ? 0.95 : 0.60 });
    if (rawJob.sourceJobUrl) candidates.push({ url: rawJob.sourceJobUrl, sourceType: 'fallback_source', suggestedConfidence: 0.40 });
    const resolvedUrls = URLResolver.resolve({ discoveryUrl: rawJob.discoveryUrl, sourceJobUrl: rawJob.sourceJobUrl, candidates });
    return Normalizer.normalize(rawJob, resolvedUrls, payloadHash);
  }

  public validate(job: NormalizedJob): JobValidationResult { return JobValidator.validate(job); }
  public async resolveApplicationUrl(candidate: JobCandidate, raw: RawJob): Promise<string> { if (raw.rawApplyUrl) return raw.rawApplyUrl; return raw.sourceJobUrl || candidate.sourceJobUrl || ''; }

  public parseMarkdownTable(markdownText: string, repoName: string, crawlDate: Date = new Date(), discoveryUrl: string = ''): { candidates: JobrightParsedRow[]; rowsParsed: number; rowsRejected: number } {
    ...
    for (const rawLine of lines) {
      ...
      if (lowerCells.some((c) => c.includes('company')) && lowerCells.some((c) => c.includes('title') || c.includes('job title'))) {
        headerIndices = { company: lowerCells.findIndex((c) => c.includes('company')), title: lowerCells.findIndex((c) => c.includes('title')), location: lowerCells.findIndex((c) => c.includes('location')), workplace: lowerCells.findIndex((c) => c.includes('work model') || c.includes('workplace') || c.includes('remote')), link: lowerCells.findIndex((c) => c.includes('link') || c.includes('apply')), date: lowerCells.findIndex((c) => c.includes('date')) };
        continue;
      }
      if (cells.every((c) => /^[-: ]+$/.test(c))) continue;
      if (!headerIndices) headerIndices = { company: 0, title: 1, location: 2, workplace: 3, link: -1, date: 4 };
      const allTextInRow = cells.join(' ');
      const linkMatch = allTextInRow.match(/https:\/\/jobright\.ai\/jobs\/info\/([a-zA-Z0-9_-]+)/);
      if (!linkMatch?.[1]) { rowsRejected++; continue; }
      ...
      let companyName = '';
      let companyWebsite = '';
      if (rawCompanyCell.includes('↳')) { companyName = lastCompany; companyWebsite = lastCompanyWebsite; }
      else {
        const compLinkMatch = rawCompanyCell.match(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/);
        if (compLinkMatch) {
          const companyText = compLinkMatch[1];
          const companyUrl = compLinkMatch[2];
          if (companyText && companyUrl) {
            companyName = companyText.replace(/[*_]/g, '').trim();
            companyWebsite = companyUrl.trim();
          }
        } else {
          companyName = rawCompanyCell.replace(/[*_]/g, '').replace(/\[([^\]]+)\]\([^)]*\)/g, '$1').trim();
        }
        if (companyName && companyName !== '↳') { lastCompany = companyName; lastCompanyWebsite = companyWebsite; }
        else { companyName = lastCompany; companyWebsite = lastCompanyWebsite; }
      }
      if (!companyName || companyName === '↳') { rowsRejected++; continue; }
      const rawTitleCell = cells[headerIndices.title] || '';
      const titleLinkMatch = rawTitleCell.match(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/);
      const titleText = titleLinkMatch?.[1] || rawTitleCell;
      const title = titleText.replace(/[*_]/g, '').replace(/\[([^\]]+)\]\([^)]*\)/g, '$1').replace(/\s+/g, ' ').trim();
      if (!title) { rowsRejected++; continue; }
      ...
      for (const cell of cells) { let match: RegExpExecArray | null; while ((match = bareUrlRegex.exec(cell)) !== null) { const clean = match[0].replace(/[.,;:!?]+$/, ''); if (!clean.includes('jobright.ai/jobs/info') && clean !== companyWebsite) bareUrls.push(clean); } }
      ...
      for (const bu of bareUrls) {
        ...
        if (isAts && !directAtsUrl && !explicitApplyUrl) directAtsUrl = bu; else otherEmbeddedUrls.push(bu);
      }
      candidates.push({ id: externalJobId, externalJobId, title, companyName, company_name: companyName, companyWebsite, location, workplaceType, postedAt, sourceJobUrl, source_job_url: sourceJobUrl, discoveryUrl, repository: repoName, original_apply_url: explicitApplyUrl, ats_url: directAtsUrl, embedded_urls: otherEmbeddedUrls.length > 0 ? otherEmbeddedUrls : undefined });
      rowsParsed++;
    }
  ...
  public parseDate(rawDate: string, crawlDate: Date = new Date()): string {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(clean)) { const d = new Date(`${clean}T00:00:00Z`); if (!isNaN(d.getTime())) return d.toISOString(); }
    ...
    if (monthDayMatch?.[1] && monthDayMatch[2]) {
      ...
      if (mIdx !== -1) { let year = crawlDate.getUTCFullYear(); if (crawlDate.getUTCMonth() < 2 && mIdx > 9) year -= 1; const d = new Date(Date.UTC(year, mIdx, day, 12, 0, 0)); if (!isNaN(d.getTime())) return d.toISOString(); }
    }
