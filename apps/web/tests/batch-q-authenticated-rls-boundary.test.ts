...
// Q-V02: HARD SECURITY GATE: Prohibit execution against production
if (
...
// Q-V01: FAIL-CLOSED: No describe.skip or conditional silent pass.
...
describe('Batch Q — Genuine Authenticated PostgREST / Multi-Tenant RLS Boundary Suite (Q-H03, Q-H04, Q-V01-Q-V05)', { timeout: 30000 }, () => {
  ...
  beforeAll(async () => {
    // Q-V01: Fail-closed requirement — must fail if credentials missing
  ...
  afterAll(async () => {
    ...
    try {
      ...
      await adminClient.from('applications').delete().in('id', [appAId, appBId, personalAppBId]);
      ...
      // Q-V02: Fail-closed verification: confirm zero leftover test fixtures
  ...
  // =========================================================================
  // Q-V02: Security Gate Verification
  // =========================================================================
  it('proves target test environment is non-production (wvyrivmvpcrhwinzmcyy) (Q-V02)', () => {
  ...
  // =========================================================================
  // Q-V03: Assignment Cancellation Is Durable and Auditable (13 Invariants)
  // =========================================================================
  it('enforces durable, non-destructive assignment cancellation with immutable lifecycle audit (Q-V03)', async () => {
    ...
    // 3. Verify through database/PostgREST boundary:
    ...
    // (6) Historical timestamps/data remain intact
    ...
    expect(current!.deadline_at).toBe(original!.deadline_at);

    // (7) Status is 'cancelled'
    expect(current!.status).toBe('cancelled');

    // (8) Assignment is no longer counted as active
    ...
    // (9) Corresponding immutable assignment_events cancellation event exists
    ...
    // (10) Event contains correct metadata
    ...
    // (11) Existing application/history is not deleted or altered
    ...
    // (12) Transitioning out of cancelled is safely rejected by database lifecycle trigger
    const { error: recancelErr } = await adminAClient
      .from('job_assignments')
      .update({ status: 'in_progress' })
      .eq('id', asgnAId);

    expect(recancelErr).not.toBeNull();
    expect(recancelErr!.message).toMatch(/INVALID_STATE_TRANSITION|terminal/i);

    // (13) Completed assignments cannot be cancelled
  ...
  // =========================================================================
  // Q-V04: Genuine Cross-Organization Sync Retry Isolation Matrix
  // =========================================================================
  it('enforces genuine cross-organization sync retry isolation matrix (Q-V04)', async () => {
    ...
    // 7. Org A Admin -> personal/non-org event of Admin B: DENY (403 Forbidden)
    const resAtoPersonalB = await SyncRetryService.executeRetry({ eventId: personalSyncEventBId }, adminAClient);
    expect(resAtoPersonalB.status).toBe(403);

    // 8. Org B Admin -> personal/non-org event of Admin B: ALLOW (200 - owner of personal event)
  ...
  // =========================================================================
  // Q-V05: Platform Admin Boundary Against Real DB
  // =========================================================================
  it('verifies platform admin (profiles.role = "admin") boundary against real DB (Q-V05)', async () => {
    ...
    // 1. Platform Admin -> Org A: ALLOW (delegated super-admin access)
    const platOrgA = await AuthGuard.requireOrgAdmin(orgAId, platformAdminClient);
    ...
    // 2. Platform Admin -> Org B: ALLOW (delegated super-admin access)
    const platOrgB = await AuthGuard.requireOrgAdmin(orgBId, platformAdminClient);
    ...
    const adminAOrgA = await AuthGuard.requireOrgAdmin(orgAId, adminAClient);
    expect('errorResponse' in adminAOrgA).toBe(false);

    // 4. Org A Admin -> Org B: DENY (403 Forbidden - cannot escape tenant)
    const adminAOrgB = await AuthGuard.requireOrgAdmin(orgBId, adminAClient);
    ...
    const adminBOrgA = await AuthGuard.requireOrgAdmin(orgAId, adminBClient);
    ...
    const adminBOrgB = await AuthGuard.requireOrgAdmin(orgBId, adminBClient);
    expect('errorResponse' in adminBOrgB).toBe(false);

    // 7. Worker A -> Org A: DENY (403 Forbidden - worker is not org admin/owner)
    const workerOrgA = await AuthGuard.requireOrgAdmin(orgAId, workerAClient);
    ...
    // 8. Worker A -> Org B: DENY (403 Forbidden - non-member)
    const workerOrgB = await AuthGuard.requireOrgAdmin(orgBId, workerAClient);
    ...
    // 9. Normal User -> Org A: DENY (403 Forbidden - non-member)
    const normalOrgA = await AuthGuard.requireOrgAdmin(orgAId, normalUserClient);
    ...
    // 10. Normal User -> Org B: DENY (403 Forbidden - non-member)
    const normalOrgB = await AuthGuard.requireOrgAdmin(orgBId, normalUserClient);
