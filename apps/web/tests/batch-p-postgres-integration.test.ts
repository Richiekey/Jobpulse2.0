...
function getSupabaseToken(): string {
  if (process.env.SUPABASE_MGMT_TOKEN) return process.env.SUPABASE_MGMT_TOKEN;
  if (process.env.SUPABASE_ACCESS_TOKEN) return process.env.SUPABASE_ACCESS_TOKEN;
  
  // Attempt to read from local gitignored env files
  const envPaths = [
...
const PROJECT_REF = process.env.SUPABASE_PROJECT_REF || 'rgwutmthzigjmzsmmjnp';
const SUPABASE_MGMT_TOKEN = getSupabaseToken();
...
describe('Batch P — Genuine PostgreSQL Multi-Tenant Integration Suite (P-H03)', () => {
  ...
  const initialNotes = 'Org A Confidential Note: Candidate requested $180k';
  ...
  beforeAll(async () => {
    // 1. Fetch real worker user and an unassigned/unapplied job for this worker
    const users = await executeSql<{ id: string }[]>(`
      SELECT id FROM auth.users ORDER BY created_at ASC LIMIT 2;
    `);
    expect(users.length).toBeGreaterThanOrEqual(1);
    workerUserId = users[0].id;
    orgBAdminId = users.length > 1 ? users[1].id : crypto.randomUUID();

    const jobs = await executeSql<{ id: string }[]>(`
      SELECT j.id 
      FROM public.jobs j
      LEFT JOIN public.applications a ON a.job_id = j.id AND a.user_id = '${workerUserId}'
      WHERE a.id IS NULL
      LIMIT 1;
    `);
    ...
    // 2. Setup Organizations, Memberships, Application A in Org A, and Assignment B in Org B atomically
    await executeSql(`
      INSERT INTO public.organizations (id, name, slug)
      VALUES 
        ('${orgAId}', 'Integration Org Alpha', 'int-org-alpha-${Date.now()}'),
        ('${orgBId}', 'Integration Org Beta', 'int-org-beta-${Date.now()}');
      ...
      -- 3. Create Application A in Org A
      ...
      -- 4. Create Assignment B in Org B
  ...
  afterAll(async () => {
    // Note: application_events and assignment_events are append-only by database design.
    // Unique test IDs ensure test isolation without violating append-only constraints.
  });
  ...
  it('verifies exact database state BEFORE RPC execution', async () => {
    ...
    // Application creation trigger produces exactly 1 initial event
    expect(parseInt(state.app_events_count, 10)).toBe(1);
    // Assignment creation trigger produces 1 assigned event, exactly 0 completed events
  ...
  it('executes genuine PostgreSQL complete_assignment_with_application RPC without mocking', async () => {
    const rpcResult = await executeSql<any[]>(`
      ...
      SELECT public.complete_assignment_with_application(
        ...
        'Stripe Technologies'::text,
        'Staff Security Engineer'::text
      ) AS result
      FROM auth_ctx;
    `);
